...
//  Async Message
//
typedef struct SecurityContextMessageCall {
  SecurityContext *context;
  SecurityBufferDescriptor *descriptor;
  unsigned long flags;
} SecurityContextMessageCall;

static void _Message(Worker *worker) {
  SECURITY_STATUS status;
  // Unpack call
  SecurityContextMessageCall *call = (SecurityContextMessageCall *)worker->parameters;
  // Unpack the security context
  SecurityContext *context = call->context;
  SecurityBufferDescriptor *descriptor = call->descriptor;

  // Let's execute ion
  status = _sspi_Message(
      &context->m_Context
    , call->flags
...
static Handle<Value> _map_Message(Worker *worker) {
  // Unwrap the security context
  SecurityContext *context = (SecurityContext *)worker->return_value;
  // Return the value
  return NanObjectWrapHandle(context);
...
NAN_METHOD(SecurityContext::Message) {
  NanScope();

  if(args.Length() != 3)
    return NanThrowError("Message takes an instance of SecurityBufferDescriptor, an integer flag and a callback function");  
  ...
  // Create call structure
  SecurityContextMessageCall *call = (SecurityContextMessageCall *)calloc(1, sizeof(SecurityContextMessageCall));
  call->context = security_context;
  ...
  worker->callback = new NanCallback(callback);
  worker->parameters = call;
  worker->execute = _Message;
  worker->mapper = _map_Message;
...
void SecurityContext::Initialize(Handle<Object> target) {
  ...
  NODE_SET_PROTOTYPE_METHOD(t, "Message", Message);
...
static LPSTR DisplaySECError(DWORD ErrCode) {
  ...
  switch(ErrCode) {
    ...
    case SEC_E__FAILURE:
      pszName = "SEC_E__FAILURE - The specified data could not be ed."; 
      break;
