 ...
 *  A Message
 */
SECURITY_STATUS SEC_ENTRY _sspi_Message(PCtxtHandle phContext, unsigned long fQOP, PSecBufferDesc pMessage, unsigned long MessageSeqNo) {
  // Create function pointer instance
  Message_fn pfn_Message = NULL;

  // Return error if library not loaded
  if(_sspi_security_dll == NULL) return -1;

  // Map function to library method
  pfn_Message = (Message_fn)GetProcAddress(_sspi_security_dll, "Message");
  ...
  // Call the function
  return (*pfn_Message)(phContext, fQOP, pMessage, MessageSeqNo);
}
