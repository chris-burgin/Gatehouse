...
Win32MongoProcessor.third_transition = function(self) {
  return function(payload, callback) {   
    var messageLength = 0;
    // Get the raw bytes
    var edBytes = new Buffer(payload, 'base64');
    var edMessage = new Buffer(messageLength);
    // Copy first byte
    edBytes.copy(edMessage, 0, 0, messageLength);
    // Set up trailer
    var securityTrailerLength = edBytes.length - messageLength;
    var securityTrailer = new Buffer(securityTrailerLength);
    // Copy the bytes
    edBytes.copy(securityTrailer, 0, messageLength, securityTrailerLength);
    ...
    var buffers = [
        new SecurityBuffer(SecurityBuffer.DATA, edBytes)
    ...
    self.security_context.decryptMessage(descriptor, function(err, security_context) {
      ...
      self.security_context.queryContextAttributes(0x00, function(err, sizes) {
        ...
        self.security_context.Message(descriptor, 0x80000001, function(err, security_context) {
          if(err) return callback(err);
          callback(null, security_context.payload);
        });
      });
