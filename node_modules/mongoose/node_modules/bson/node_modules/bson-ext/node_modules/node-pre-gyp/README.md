...
#### 4) Create secure variables
 ...
 your S3 AWS keys by going to <https://ci.appveyor.com/tools/> and hitting the `` button.
...
NOTE: keys are per account but not per repo (this is difference than travis where keys are per repo but not related to the account used to  them).
...
Use `travis-` like:
    ...
    travis  node_pre_gyp_accessKeyId=${node_pre_gyp_accessKeyId}
    travis  node_pre_gyp_secretAccessKey=${node_pre_gyp_secretAccessKey}
...
More details on travis ion at http://about.travis-ci.org/docs/user/ion-keys/.
